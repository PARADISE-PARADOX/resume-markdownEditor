...
export class ParserInline {
    ...
    parseInline(text: string): InlineToken[] {
        ...
        while (remainText) {
            ...
            for (const { type, regex } of ParserInline.nonGlobalRules) {
                ...
                if (matched && (!earliestMatch || matched.index < earliestMatch.index)) {
            ...
            if (earliestMatch.index > 0) {
                tokens.push({ type: 'text', value: remainText.slice(0, earliestMatch.index) });
            }

            const matchGroup = earliestMatch.match.slice(1);
            tokens.push(this.tokenToHtml(earliestMatch.type, matchGroup));

            remainText = remainText.slice(earliestMatch.index + earliestMatch.match[0].length);
        }
    ...
    private tokenToHtml(type: InlineType, group: string[]): InlineToken {
        ...
        switch (type) {
            ...
            case 'link':
                return { type: 'link', url: group[1] || '', nesting: this.parseInline(group[0] || '') };
            case 'inlineCode':
            ...
            case 'superscript':
            case 'subscript':
                return { type, nesting: this.parseInline(content) };
            default:
