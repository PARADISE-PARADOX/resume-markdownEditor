...
export class ParserBlock {
  ...
  private createParagraph(lines: string[]): ASTNode {
    ...
    if (hasBlankLine) {
      return {
        type: "paragraph",
        nesting: this.inLineParser.parseInline(lines.join(" ")),
      };
    }
  ...
  private handleBlcokquote(
    ...
    while (
      ...
      const [, content] =
        lines[curIndex].match(rules.markdown.blockquote) || [];
      quotes.push(content);
      curIndex++;
    }
  ...
  parserBlocks(lines: string[]): ASTNode[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (rules.markdown.list.test(line)) {
        ...
        blocks.push({
          type: "listItem",
          ordered: !!order.match(/\d+\./), //如果有order，就为true，否则为false
